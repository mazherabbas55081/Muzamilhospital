...
def ai_assistant_page():
    st.markdown("## 🤖 Al Muzamil AI Assistant")
    st.caption("Powered by Groq • General hospital information assistant")

    if Groq is None:
        st.error("Groq package is not installed. Add groq to requirements.txt and redeploy.")
        return
    ...
    if not api_key:
        st.warning("Groq API key is not configured yet.")
        st.info("After deploying, add GROQ_API_KEY in Streamlit → App settings → Secrets. Do not put the key inside app.py or GitHub.")
        return
    ...
    prompt = st.chat_input("Ask about departments, services, appointments, lab tests, etc.")
    if prompt:
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": (
                        "You are Al Muzamil Hospital's helpful information assistant. "
                        "Answer clearly and briefly. You may explain hospital services, departments, "
                        "appointments and general health information. Do not diagnose patients or "
                        "replace a qualified doctor. For emergencies, tell the user to contact local "
                        "emergency services or the hospital emergency department."
                    ),
                }
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    ...
                    answer = response.choices[0].message.content or "Sorry, I could not generate a response."
                    st.markdown(answer)
            st.session_state.ai_messages.append({"role": "assistant", "content": answer})
        except Exception as e:
            st.error(f"Groq AI error: {e}")

    if st.session_state.ai_messages and st.button("🗑️ Clear AI Chat"):
        st.session_state.ai_messages = []
        st.rerun()
...
def main():
    ...
    pages = [
        "🏠 Home", "🏥 Facilities", "🤖 AI Assistant", "🏢 Departments", "👨‍⚕️ Doctors",
